      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 flex-1 flex flex-col overflow-hidden w-full">
        <form onSubmit={(e) => { e.preventDefault(); handleAssign() }} className="flex-1 flex flex-col h-full">
          <div className="flex flex-1 overflow-hidden divide-x divide-slate-200">
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar">
                {students.map((s) => (
                  <div
                    ...
                    className={`flex items-center justify-between p-3 rounded-xl border cursor-pointer transition-all group ${selectedStudents.includes(s.user_id)
                  ...
                  >
                    ...
                    <div
                      className={`w-6 h-6 rounded-full flex items-center justify-center transition-colors ${selectedStudents.includes(s.id)
            ...
            { }
            <div
              className={`flex-1 flex flex-col min-w-0 transition-opacity ${selectedStudents.length === 0 ? "opacity-50 pointer-events-none bg-slate-50" : "bg-white"}`}
            >
              <div className="p-4 border-b border-slate-100 bg-white">
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-2 relative custom-scrollbar">
                ...
                {courses.map((c) => (
                  <div
                    ...
                    className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all group ${selectedCourses.includes(c.courses_id)
                      ? "bg-indigo-50 border-[var(--color-indigo-600)] shadow-sm"
                      : "bg-white border-slate-200 hover:border-indigo-300 hover:shadow-sm"
                      }`}
                  >
                    <div
                      className={`w-10 h-10 rounded-lg flex items-center justify-center transition-colors shrink-0 ${selectedCourses.includes(c.courses_id)
                    ...
                    <div
                      className={`w-6 h-6 rounded-full flex items-center justify-center transition-colors ${selectedCourses.includes(c.courses_id)
                        ? "text-[var(--color-indigo-600)]"
                        : "text-slate-200 group-hover:text-indigo-300"
                        }`}
                    >
