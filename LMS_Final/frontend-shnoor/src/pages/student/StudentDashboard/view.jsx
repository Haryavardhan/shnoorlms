...
import { BookOpen, Flame, Trophy, Play, Clock, ArrowRight, Zap, Sparkles, Gem } from 'lucide-react';

const StudentDashboardView = ({ studentName, enrolledCount, lastCourse, gamification, recentActivity = [], deadlines = [], recommendedCourses = [], navigate }) => {
    return (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 space-y-8">
                    ...
                    {recommendedCourses && recommendedCourses.length > 0 && (
                        <div className="bg-white border border-slate-200 rounded-lg shadow-sm p-6 relative overflow-hidden mb-8">
                            <div className="flex items-center justify-between mb-4">
                                <div className="flex items-center gap-2">
                                    <Sparkles className="text-amber-500" size={16} fill="currentColor" />
                                    <h3 className="text-sm font-bold text-slate-500 uppercase tracking-wide">Recommended Next Steps</h3>
                                </div>
                            </div>

                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {recommendedCourses.map(course => (
                                    <div key={course.id || course.courses_id} className="border border-slate-200 rounded-lg p-4 hover:border-indigo-400 transition-colors cursor-pointer group bg-slate-50/50" onClick={() => navigate(`/student/course/${course.id || course.courses_id}`)}>
                                        <div className="flex justify-between items-start mb-2">
                                            <div className="bg-white text-[10px] font-bold px-2 py-1 rounded border border-slate-200 shadow-sm uppercase tracking-wide text-slate-600 group-hover:text-indigo-600 transition-colors">
                                                {course.level}
                                            </div>
                                            {course.is_paid ? (
                                                <div className="flex items-center gap-1 text-[10px] font-bold text-amber-600 bg-amber-50 px-2 py-1 rounded border border-amber-100">
                                                    <Gem size={10} /> ₹{course.price}
                                                </div>
                                            ) : (
                                                <div className="text-[10px] font-bold text-emerald-600 bg-emerald-50 px-2 py-1 rounded border border-emerald-100">
                                                    FREE
                                                </div>
                                            )}
                                        </div>
                                        <h4 className="font-bold text-primary-900 text-sm mb-1 line-clamp-2 leading-snug group-hover:text-indigo-700">{course.title}</h4>
                                        <p className="text-xs text-slate-500 mb-3 line-clamp-2">{course.description}</p>
                                        <div className="flex items-center gap-1 text-xs font-bold text-indigo-600 group-hover:underline">
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-gradient-to-br from-indigo-900 to-primary-900 rounded-lg p-6 text-white relative overflow-hidden shadow-lg">
                        <div className="relative z-10">
                            <h3 className="font-bold text-lg mb-2">Weekly Contests</h3>
                            <p className="text-indigo-200 text-sm mb-4">Compete with peers and win exclusive rewards.</p>
                            <button
                        ...
                        <Trophy className="absolute right-[-20px] bottom-[-20px] text-yellow-400/20" size={120} />
                    </div>
                </div>
            </div>
        </div>
